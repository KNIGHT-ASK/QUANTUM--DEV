...
export class HilbertSpace {
	 ...
	 * ||ψ||² = Σᵢ|αᵢ|² = 1
	 */
	validateNormalization(state: Complex[]): { isValid: boolean; norm: number; error?: string } {
	...
	calculateVonNeumannEntropy(densityMatrix: Complex[][]): number {
		...
		for (const lambda of eigenvalues) {
			const lambdaVal = typeof lambda === 'number' ? lambda : (lambda as any).re;
	...
	private stateToDensityMatrix(state: Complex[]): Complex[][] {
		...
		for (let i = 0; i < n; i++) {
			rho[i] = [];
			for (let j = 0; j < n; j++) {
				// ρᵢⱼ = αᵢ αⱼ* (outer product |ψ⟩⟨ψ|)
				const alphaI = math.complex(state[i]);
				const alphaJConj = math.conj(math.complex(state[j])) as Complex;
				rho[i][j] = math.multiply(alphaI, alphaJConj) as Complex;
			}
		}
	...
	private trace(matrix: Complex[][]): number {
		let tr = 0;
	...
	private getEigenvalues(matrix: Complex[][]): number[] {
		...
		if (n === 2) {
			...
			const bcValue = typeof bc === 'number' ? bc : (bc as any).re || 0;
	...
	private singularValueDecomposition(matrix: Complex[][]): number[] {
