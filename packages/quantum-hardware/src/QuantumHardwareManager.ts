...
export interface QubitTopology {
    ...
    numQubits: number;
    connectivity: Map<number, number[]>; // qubit -> connected qubits
    couplingMap: [number, number][]; // List of connected qubit pairs
    nativeGates: string[]; // Gates directly supported
    basisGates: string[]; // Gate set for compilation
}
...
export interface QubitCalibration {
    qubitId: number;
    t1: number; // Relaxation time (microseconds)
    t2: number; // Dephasing time (microseconds)
    frequency: number; // Qubit frequency (GHz)
    anharmonicity: number; // Anharmonicity (MHz)
    gateErrors: Map<string, number>; // gate name -> error rate
    readoutError: number; // Measurement error rate
    readoutAssignmentError: [number, number]; // [P(0|1), P(1|0)]
}
...
export interface QuantumDevice {
    ...
    maxExperimentTime: number; // microseconds
    processorType: 'superconducting' | 'ion_trap' | 'photonic' | 'neutral_atom';
    lastCalibrated: Date;
}

export class QuantumHardwareManager {
    private devices: Map<string, QuantumDevice> = new Map();
    private currentDevice: QuantumDevice | null = null;
     ...
     */
    private loadPredefinedDevices(): void {
     ...
     */
    private createIBMBrisbane(): QuantumDevice {
        const numQubits = 127;
        ...
        for (let i = 0; i < numQubits; i++) {
            ...
            // Add to coupling map
            neighbors.forEach(n => {
                if (n > i) couplingMap.push([i, n]);
            });
        }

        const qubitCalibrations = new Map<number, QubitCalibration>();
        for (let i = 0; i < numQubits; i++) {
     ...
     */
    private createIBMKyoto(): QuantumDevice {
     ...
     */
    private createIonQAria(): QuantumDevice {
        const numQubits = 25;
        const connectivity = new Map<number, number[]>();
        const couplingMap: [number, number][] = [];
     ...
     */
    private createRigettiAspenM3(): QuantumDevice {
        const numQubits = 79;
        const connectivity = new Map<number, number[]>();
        const couplingMap: [number, number][] = [];
        ...
        for (let i = 0; i < numQubits; i++) {
            ...
            if (i < numQubits - 1) neighbors.push(i + 1);
     ...
     */
    private createGoogleSycamore(): QuantumDevice {
        const numQubits = 53;
        const connectivity = new Map<number, number[]>();
        const couplingMap: [number, number][] = [];
        ...
        for (let i = 0; i < numQubits; i++) {
            ...
            if (col > 0) neighbors.push(i - 1);
            // Right
            if (col < gridWidth - 1 && i + 1 < numQubits) neighbors.push(i + 1);
            // Up
            if (row > 0) neighbors.push(i - gridWidth);
            // Down
            if (i + gridWidth < numQubits) neighbors.push(i + gridWidth);
     ...
     */
    estimateCircuitFidelity(gates: { name: string; qubits: number[] }[]): number {
